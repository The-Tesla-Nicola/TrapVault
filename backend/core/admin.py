...
from .models import (
    MonitorUser, AttackerSession, AttackEvent, CapturedCredential,
    DeceptionAsset, DeceptionInteraction, MonitorAuditLog,
    RealBankUser, SiemAlert, LoginAttempt, AlertRule,
)
...
class MonitorUserAdmin(UserAdmin):
    ...
    fieldsets = UserAdmin.fieldsets + (
        ('Security Role', {'fields': (
            'role', 'mfa_enabled', 'last_login_ip',
...
class RealBankUserAdmin(admin.ModelAdmin):
    list_display  = ['username', 'full_name', 'email', 'account_number', 'is_active', 'last_login']
    list_filter   = ['is_active']
    search_fields = ['username', 'email', 'full_name', 'account_number']
    readonly_fields = ['id', 'password_hash', 'created_at', 'last_login', 'last_login_ip']


@admin.register(SiemAlert)
class SiemAlertAdmin(admin.ModelAdmin):
    list_display  = ['timestamp', 'attack_type', 'severity', 'confidence',
                     'session_score', 'routing_decision', 'is_acknowledged']
    list_filter   = ['severity', 'attack_type', 'routing_decision', 'is_acknowledged',
                     'is_brute_force', 'is_burst']
...
class LoginAttemptAdmin(admin.ModelAdmin):
    list_display  = ['timestamp', 'ip_address', 'username', 'outcome',
                     'attack_type', 'session_score', 'confidence']
...
class AlertRuleAdmin(admin.ModelAdmin):
    list_display = ['name', 'match_severity', 'match_attack_type',
                    'notification_channel', 'auto_block', 'is_active']
...
class AttackerSessionAdmin(admin.ModelAdmin):
    list_display  = ['ip_address', 'country_code', 'threat_level', 'threat_score',
                     'total_requests', 'is_blocked', 'first_seen']
...
class CapturedCredentialAdmin(admin.ModelAdmin):
    list_display  = ['timestamp', 'session', 'username', 'credential_type',
                     'is_default_credential', 'password_strength']
