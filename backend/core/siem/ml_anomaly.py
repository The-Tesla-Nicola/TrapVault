...
class MLAnomalyDetector:
    ...
    def predict_anomaly_score(self, session_id):
        ...
        if features is None:
            return {'is_anomaly': False, 'anomaly_score': 0.0, 'confidence': 0.0, 'status': 'insufficient_data'}
    ...
    def _fallback_scoring(self, features):
        request_freq, payload_avg, payload_std, unique_paths, error_rate, suspicious_rate = features
        score = 0.0
        if request_freq > 20: score += 30
        elif request_freq > 10: score += 15
        if payload_std > 500: score += 20
        elif payload_std > 200: score += 10
        if unique_paths > 15: score += 25
        elif unique_paths > 8: score += 12
        if error_rate > 0.5: score += 15
        elif error_rate > 0.3: score += 8
        if suspicious_rate > 0.5: score += 30
        elif suspicious_rate > 0.2: score += 15
