...
class ThreatAnalyzer:
    ...
    XSS_PATTERNS: List[Tuple[str, str, float]] = [
        ...
        (r"on\w+\s*=\s*['\"]?\s*(javascript|alert|eval|document)", 'xss_event_handler', 0.90),
    ...
    COMMON_PASSWORDS: List[str] = [
        ...
        'baseball', 'iloveyou', 'master', 'sunshine', 'welcome',
        'passw0rd', 'shadow', '123123', '654321', '1234',
        'password1', 'qwerty123', 'admin', 'changeme', 'default',
    ]
    ...
    def analyze(self, event) -> Dict[str, Any]:
        ...
        recon_score, recon_patterns = self._check_patterns(path_lower, self.RECON_PATTERNS)
        ...
        if ('/login' in path_lower or '/auth' in path_lower) and getattr(event, 'method', '') == 'POST':
