...
check_body \
    "POST /api/auth/login/ – real customer (alice.johnson)" \
    "200" "POST" "$BASE_URL/api/auth/login/" \
    "success" \
    -H "Content-Type: application/json" \
    -d '{"username":"alice.johnson","password":"BankPass!2024"}'
...
check_body \
    ...
    "error" \
    -H "Content-Type: application/json" \
    -d '{"username":"alice.johnson","password":"wrongpassword"}'
...
if [ -z "$MONITOR_TOKEN" ]; then
    ...
    warn "Check: make create-user USER=cyber_admin PASS='CyB3r_P@ssw0rd!99' ROLE=admin"
else
